...
class PrometheusServiceProvider extends ServiceProvider
{
    public function register()
    {
        ...
        Prometheus::addGauge('Total Users')
            ->helpText('The total number of registered users')
            ->value(fn() => User::count());

        // 2. Monitor Successful Logins (Counter simulation using Gauge)
        // You would increment a value in Redis whenever a login succeeds
        Prometheus::addGauge('Successful Logins')
            ->value(fn() => cache()->get('auth_success_logins', 0));
